...
while ($true)
{
  ...
  {
    $messagel6 = $messages | Sort ReceivedTime | Select-Object -Last $lastNCount
    foreach ($message in $messagel6)
    {
      ...
      if ($subject.Contains("DPH") -eq $false)
      {
        ...
        $reresults = $sourcepath.Matches.Value | Select-Object -Last 1
        ...
        $reresults = $buildnumber.Matches.Value | Select-Object -Last 1
